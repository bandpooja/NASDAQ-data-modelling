...
class NASDAQModel:
    ...
    def fit(self, x_train: pd.DataFrame, y_train: pd.DataFrame):
        ...
        # fixing the max depth to make the fitting easier
        self.model = RandomForestRegressor(n_estimators=100, max_depth=50, random_state=42)

        # fitting the model with dask parallelization
        with joblib.parallel_backend('dask'):
            self.model.fit(x_train, y_train)

        # release the memory from the workers
        client.close()
    ...
    def test(self, x_test: pd.DataFrame, y_test: pd.DataFrame) -> None:
        ...
        logger.info("-" * 10)
        return None  # y_test, y_pred, round(r2_score(y_test, y_pred), 4)
    ...
    def predict(self, vol_moving_avg: float, adj_close_rolling_med: float) -> float:
        ...
        return self.model.predict(self.standard_scaler.transform(df)).tolist()[0]
...
if __name__ == "__main__":
    ...
    '''
    y_gt, y_pred, R2 = m.test(x_test, y_test)
    import matplotlib.pyplot as plt
    plt.scatter(y_gt, y_pred)
    plt.plot([min(min(y_gt), min(y_pred)), max(max(y_gt), max(y_pred))],
        [min(min(y_gt), min(y_pred)), max(max(y_gt), max(y_pred))],
        'k--', label=f"R2: {R2}")
    plt.xlabel("y-gt, Ground Truth")
    plt.ylabel("y-pred, Prediction")
